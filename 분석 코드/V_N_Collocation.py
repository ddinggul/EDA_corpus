...
# 'V + N' 형태의 collocation 추출 함수
def extract_verb_noun_collocations(tagged_words):
    print("Extracting V + N collocations...")
    collocations = []
    for i in range(len(tagged_words) - 1):
        if tagged_words[i][1] in ['VB', 'VBD', 'VBG', 'VBN', 'VBP', 'VBZ'] and tagged_words[i + 1][1] in ['NN', 'NNS',
                                                                                                          'NNP',
                                                                                                          'NNPS']:
            collocations.append((tagged_words[i][0], tagged_words[i + 1][0]))
    print("V + N collocations extracted.")
    return collocations
...
collocations_output_file = '/Users/junseo/PycharmProjects/Corpus_Analysis/vn_collocations_output.xlsx'
...
for filename in os.listdir(learners_directory):
    ...
    # 'V + N' 형태의 collocation 추출
    verb_noun_collocations = extract_verb_noun_collocations(tagged_words)
    learners_collocations.extend(verb_noun_collocations)
...
for filename in os.listdir(native_directory):
    ...
    # 'V + N' 형태의 collocation 추출
    verb_noun_collocations = extract_verb_noun_collocations(tagged_words)
    native_collocations.extend(verb_noun_collocations)
